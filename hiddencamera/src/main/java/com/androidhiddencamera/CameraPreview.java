...
class CameraPreview extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public void surfaceChanged(SurfaceHolder surfaceHolder, int i, int i1, int i2) {
        ...
        try {
            ...
            // Ignore: tried to stop a non-existent preview 尝试停止不存在的预览
        }

        // Make changes in preview size 改变预览尺寸
        Camera.Parameters parameters = mCamera.getParameters();
        List<Camera.Size> pictureSizes = mCamera.getParameters().getSupportedPictureSizes();

        //Sort descending 下行排序
        Collections.sort(pictureSizes, new PictureSizeComparator());

        //set the camera image size based on config provided 设置照相机尺寸基于提供的配置
