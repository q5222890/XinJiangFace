package com.zack.xjht.face.faceserver;
...
import com.zack.xjht.face.model.FaceRegisterInfo;
        ...
        synchronized (this) {
            if (faceEngine == null && context != null) {
                ...
                int engineCode = faceEngine.init(context, DetectMode.ASF_DETECT_MODE_IMAGE,
                        DetectFaceOrientPriority.ASF_OP_0_ONLY, 16,
    ...
    public int clearAllFaces(Context context) {
        synchronized (this) {
            ...
            if (featureFileDir.exists() && featureFileDir.isDirectory()) {
                ...
                Log.i(TAG, "clearAllFaces featureFiles length: "+featureFiles.length);
                if (featureFiles != null && featureFiles.length > 0) {
                    for (File featureFile : featureFiles) {
...
//                Log.i(TAG, "clearAllFaces imgFiles length: "+imgFiles.length);
...
//            return deletedFeatureCount > deletedImageCount ? deletedImageCount : deletedFeatureCount;
     ...
     * @return
     */
    public boolean deleteFace(Context context, String faceId) {
        synchronized (this) {
            if (context == null) {
                return false;
            }
            ...
            if (featureFileDir.exists() && featureFileDir.isDirectory()) {
                File[] featureFiles = featureFileDir.listFiles();
                Log.i(TAG, "deleteFace featureFiles length: "+featureFiles.length);
                if (featureFiles != null && featureFiles.length > 0) {
                    for (File featureFile : featureFiles) {
                        if (featureFile.getName().equals(faceId)) {
                            if(featureFile.delete()){
                                return true;
                            }
                        }
...
//
                    ...
                    onRegisterResultListener.onRegisterResult(name, faceFeature.getFeatureData(), true);
                    return true;
                } catch (IOException e) {
                    e.printStackTrace();
                    onRegisterResultListener.onRegisterResult(name, new byte[]{}, false);
    ...
    public boolean saveFaceFeature(Context context, byte[] featureData, String name){
        if (ROOT_PATH == null) {
            ROOT_PATH = context.getFilesDir().getAbsolutePath();
        }
        //特征存储的文件夹
    ...
    public void setOnRegisterResultListener(FaceServer.onRegisterResultListener onRegisterResultListener) {
        this.onRegisterResultListener = onRegisterResultListener;
    }

    public interface onRegisterResultListener{
        void onRegisterResult(String id, byte[] feature, boolean success);
    }
}
